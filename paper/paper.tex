...
\title[Fourier Phase Space as a Candidate Code for Musical Pitch]{Frequency Range of
Fourier Phase Space as a Candidate Code for Musical Pitch Structure: Evidence from
Artificial Neural Network Models of Iranian Classical Music}

\author[1]{\fnm{Yousef} \sur{Aftabi}}\email{yousef.aftabi@uok.ac.ir} % ایمیل نویسنده اول (در صورت نیاز)

\author*[1]{\fnm{Hamid} \sur{Farvaresh}}\email{farvaresh@uok.ac.ir} % نویسنده مسئول

\affil[1]{\orgdiv{Department of Psychology}, \orgname{University of Kurdistan},
\orgaddress{\city{Sanandaj}, \country{Iran}}}

%\affil[2]{\orgdiv{Department of Cognitive Neuroscience}, \orgname{University of Tabriz},
%\orgaddress{\city{Tabriz}, \country{Iran}}}
...
melodic expectation \cite{sankaran2024}. If the human auditory and cognitive system
genuinely partitions musical information by frequency in this way, a representational
scheme built explicitly around frequency components, such as the Discrete Fourier
Transform, is a natural computational candidate for the format such partitioning might
take, independent of whether any particular implementation in artificial neural networks
...
targets the acquisition of scale schemas from melodic sequences, whereas the present study
targets the representational format that emerges once interval and tetrachord categories
have already been learned. The two approaches are complementary tests of the same broader
...
and consolidated \cite{perez2025}. The finding is not isolated to one architecture or
learning algorithm: a closely related convergence has since been reported in generative
models, where the latent space learned by variational autoencoders trained on tonal
corpora shows substantial rank correlation with DFT-derived phase spaces
\cite{rodrigues2024}, suggesting that Fourier phase space may be a representation that
multiple, architecturally distinct learning systems independently discover. Separately,
...
across listeners or musical systems. Taken together, this is a strong and, in principle,
falsifiable claim: if the DFT is genuinely a privileged candidate code for musical
cognition rather than a code specific to the structure of Western tonal music, it should
also characterize the representations that networks discover when trained on musical
entities from an unrelated, independently evolved musical culture. The present study tests
...
covering both melodic intervals and Dangs (tetrachords); describe the architecture and
training of the ANNs used to solve them; and construct a principled set of Fourier phase
spaces over the Iranian pitch-class lattice, applying the identical construction to a
...
system. This motivates a hypothesis we consider but do not treat as fully established: that
different musical cultures may be distinguished, at a computational and possibly cognitive
level, by different frequency ranges of a shared Fourier-based representational code.
...
a separate generator seeded at 0. Every random draw is therefore exactly reproducible: weight
...
\ednote{insert repository URL / archived DOI}.
...
directly and, crucially, well above its own chance level in both systems (Iranian mean \num{0.545} versus a chance level of \num{0.254}; Western \num{0.905}
...
networks trained on both cultures converge on a broadly similar representational format: a
small number of Fourier phase spaces, each a good descriptor of a hidden unit's encoding.
...
\section{Conclusion}
Artificial neural networks trained to solve classification problems from Iranian classical
music---a microtonal tradition whose 17 pitch classes lie on a 24-tone quarter-tone lattice
and are not a subset of the Western chromatic scale---reliably develop connection weights
dominated by a single Fourier phase space. This dominance survives false-discovery-rate
...
\ednote{repository URL / archived DOI}.
...
analysis outputs are openly available \ednote{repository URL / archived DOI}.

\item \textbf{Materials availability.} Not applicable.

\item \textbf{Code availability.} The full simulation and analysis pipeline is openly
available \ednote{repository URL / archived DOI}.
